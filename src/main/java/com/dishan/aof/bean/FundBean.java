...
public class FundBean {
    ...
    public String getValueByColumn(String colums, boolean colorful) {
        switch (colums) {
            ...
            case "上区间":
                return findLastInterval(gsz, fundCode)[0];
            case "下区间":
                return findLastInterval(gsz, fundCode)[1];
            case "建仓":
                return findGridStrategyStr(fundCode)[0];
            case "上梯度":
                return findGridStrategyStr(fundCode)[1];
            case "下梯度":
                return findGridStrategyStr(fundCode)[2];
            case "上次买":
                return OperateTraceConfigItem.findLastOperate(fundCode)[0];
            case "上次卖":
                return OperateTraceConfigItem.findLastOperate(fundCode)[1];
            default:
                return "";
