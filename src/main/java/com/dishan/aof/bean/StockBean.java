...
import static com.dishan.aof.setting.GridStrategyConfig.findGridStrategyStr;
import static com.dishan.aof.setting.GridStrategyConfig.findLastInterval;
        ...
        switch (colums) {
            ...
            case "更新时间":
                ...
                if (this.getTime() != null) {
                    Date date = DateUtils.parseDate(this.getTime(), new String[]{"yyyyMMddHHmmss", "yyyy/MM/dd HH:mm:ss", "yyyy-MM-dd HH:mm:ss"});
                    timeStr = DateFormatUtils.format(date,"MM-dd HH：mm");
                }
                return timeStr;
            case "上区间":
                return findLastInterval(getNow(), code)[0];
            case "下区间":
                return findLastInterval(getNow(), code)[1];
            case "建仓":
                return findGridStrategyStr(code)[0];
            case "上梯度":
                return findGridStrategyStr(code)[1];
            case "下梯度":
                return findGridStrategyStr(code)[2];
            case "上次买":
                return OperateTraceConfigItem.findLastOperate(code)[0];
            case "上次卖":
                return OperateTraceConfigItem.findLastOperate(code)[1];
            default:
                return "";
