...
import com.dishan.aof.bean.FundBean;
...
public abstract class FundRefreshHandler extends DefaultTableModel {
    ...
    static {
        ...
        String tableHeader = instance.getValue(TableConstants.FUND_TABLE_HEADER_KEY);
        if (StringUtils.isBlank(tableHeader)) {
            instance.setValue(TableConstants.FUND_TABLE_HEADER_KEY, TableConstants.FUND_TABLE_HEADER_VALUE);
            tableHeader = TableConstants.FUND_TABLE_HEADER_VALUE;
        }
    ...
    public void refreshColorful(boolean colorful) {
        ...
        Arrays.stream("估算净值,估算涨跌".split(","))
                .map(name -> TableConstants.getColumnIndexByName(columnNames, name))
