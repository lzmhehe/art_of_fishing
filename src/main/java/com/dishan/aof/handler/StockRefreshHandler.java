...
public abstract class StockRefreshHandler extends DefaultTableModel {
    ...
    static {
        ...
        String tableHeaderValue = instance.getValue(TableConstants.STOCK_TABLE_HEADER_KEY);
        if (StringUtils.isBlank(tableHeaderValue)) {
            instance.setValue(TableConstants.STOCK_TABLE_HEADER_KEY, TableConstants.STOCK_TABLE_HEADER_VALUE);
            tableHeaderValue = TableConstants.STOCK_TABLE_HEADER_VALUE;
        }
    ...
    public void refreshColorful(boolean colorful) {
        ...
        Arrays.stream("当前价,涨跌,涨跌幅,最高价,最低价".split(",")).map(name -> TableConstants.getColumnIndexByName(columnNames, name))
    ...
    private void columnColors(boolean colorful) {
        ...
        int columnIndex1 = TableConstants.getColumnIndexByName(columnNames, "涨跌");
        int columnIndex2 = TableConstants.getColumnIndexByName(columnNames, "涨跌幅");
